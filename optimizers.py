...
class H_SA_SGHMC(BaseOtimizer):
    ...
    def __init__(self, params, lr=1e-2, base_C=0.05, gauss_sig=0.1, alpha0=10, beta0=10):
        ...
        self.weight_decay = gamma(shape=alpha0, scale=1 / (beta0), size=None)
...
class SGLD(BaseOtimizer):
    """
    SGLD optimiser based on pytorch's SGD.
    Note that the weight decay is specified in terms of the gaussian prior sigma.
    """

    def __init__(self, params, lr=required, alpha0=10, beta0=10, addnoise=True):

        self.alpha0 = alpha0
        self.beta0 = beta0
        self.weight_decay = Gamma(concentration=alpha0, rate=1 / (beta0)).sample(torch.Size([1]))
    ...
    def step(self, resample_prior=False, closure=None, **kwargs):
        """
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if resample_prior:
                    ...
                    gamma_sample = Gamma(concentration=alpha, rate=1 / (beta)).sample(torch.Size([1]))

                    state['weight_decay'] = gamma_sample

                d_p = p.grad.data
                if weight_decay != 0:
                    d_p.data.add_(p, alpha=weight_decay.item())
