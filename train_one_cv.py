...
def get_cv(psy_input_dim, args, prior=None, potential_grad=None, 
...
def test_cv(trajs, ncv, batches, args):
    n_traj = len(trajs)
        ...
        for _ in range(args.n_cv_iter):
            ...
            for (models, _, potential_grad) in trajs:
                if args.var_estimator == 'sample':
                    var_estimator = SampleVarianceEstimator(function_f, models)
                else:
                    var_estimator = SpectralVarianceEstimator(function_f, models)
...
def main(args):
    ...
    trajs = list(zip(trajectories, priors, potential_grads))
    train_trajs, test_trajs, _, _ = train_test_split(trajs, [None]*len(trajs), test_size=len(trajs)-args.n_train_traj)
    ...
    ncv = get_cv(psy_input_dim, args)
    train_cv(train_trajs, ncv, batches, args)
    test_cv(test_trajs, ncv, batches, args)
